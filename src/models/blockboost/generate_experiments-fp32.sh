...
proportion_nonmatches=( 16 8 )

mkdir -p out/blockboost-fp32/train
echo "Training (outputs in out/blockboost-fp32/train)..."

start_time=$SECONDS
parallel --halt-on-error 2 -j8 --progress \
    'OMP_NUM_THREADS=36 ./bin/models/blockboost-fp32/train-omp {2} {3} 1 {1} 5000 &> out/blockboost-fp32/train/{#}.job' \
...
src/eval/eval.sh blockboost-fp32
