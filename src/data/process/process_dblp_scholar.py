...
raw_dir = "research/data/raw/dblp_scholar/"
zip_file = raw_dir + "data-2022-10-04T15:16:42.135-03:00.zip"
...
output_dir = output_dir / "dblp_scholar"
output_dir.mkdir(parents=True, exist_ok=True)
...
matches["idDBLP"] = "id_A-" + matches["idDBLP"].astype(str)
matches["idScholar"] = "id_B-" + matches["idScholar"].astype(str)

# assert ids from both datasets are unique
assert len(set(df_A["id"].values).intersection(set(df_B["id"].values))) == 0


idA_counted = matches["idDBLP"].value_counts()
duplicate_A_ids = idA_counted[idA_counted >= 2].index.values

idB_counted = matches["idScholar"].value_counts()
...
matches = matches[~matches["idDBLP"].isin(duplicate_A_ids)]
matches = matches[~matches["idScholar"].isin(duplicate_B_ids)]
...
for B_id in B_ids:
    ...
    B_match = matches[matches["idScholar"] == B_id]

    if B_match.shape[0] == 1:
        id_to_entity_id[B_id] = B_match["idDBLP"].values[0]
    else:
