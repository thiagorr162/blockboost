...
raw_dir = "research/data/raw/walmart_amazon/"
zip_file = raw_dir + "data-2022-10-04T15:16:55.754-03:00.zip"
...
output_dir = output_dir / "walmart_amazon"
output_dir.mkdir(parents=True, exist_ok=True)
...
df_A = df_A.rename({"custom_id": "id"}, axis=1)
df_B = df_B.rename({"custom_id": "id"}, axis=1)
...
matches["id1"] = "id_A-" + matches["id1"].astype(str)
matches["id2"] = "id_B-" + matches["id2"].astype(str)
...
idA_counted = matches["id1"].value_counts()
duplicate_A_ids = idA_counted[idA_counted >= 2].index.values

idB_counted = matches["id2"].value_counts()
duplicate_B_ids = idB_counted[idB_counted >= 2].index.values

# print(json.dumps(duplicate_A_ids.tolist()))
# print(json.dumps(duplicate_B_ids.tolist()))

df_A = df_A[~df_A["id"].isin(duplicate_A_ids)]
df_B = df_B[~df_B["id"].isin(duplicate_B_ids)]

matches = matches[~matches["id1"].isin(duplicate_A_ids)]
matches = matches[~matches["id2"].isin(duplicate_B_ids)]
...
with open(raw_dir + "matches.json", "w") as f:
    json.dump(matches_full, f)
...
for B_id in B_ids:
    ...
    B_match = matches[matches["id2"] == B_id]

    if B_match.shape[0] == 1:
        id_to_entity_id[B_id] = B_match["id1"].values[0]
    else:
