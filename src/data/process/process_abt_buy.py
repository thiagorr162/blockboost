...
raw_dir = "research/data/raw/abt_buy/"
zip_file = raw_dir + "data-2022-10-04T15:15:30.545-03:00.zip"
...
df_A["price"] = df_A["price"].str.replace(",", "")
...
matches["idAbt"] = "id_A-" + matches["idAbt"].astype(str)
matches["idBuy"] = "id_B-" + matches["idBuy"].astype(str)

# assert ids from both datasets are unique
assert len(set(df_A["id"].values).intersection(set(df_B["id"].values))) == 0


idA_counted = matches["idAbt"].value_counts()
duplicate_A_ids = idA_counted[idA_counted >= 2].index.values

idB_counted = matches["idBuy"].value_counts()
...
matches = matches[~matches["idAbt"].isin(duplicate_A_ids)]
matches = matches[~matches["idBuy"].isin(duplicate_B_ids)]
...
for B_id in B_ids:
    ...
    B_match = matches[matches["idBuy"] == B_id]

    if B_match.shape[0] == 1:
        id_to_entity_id[B_id] = B_match["idAbt"].values[0]
    else:
