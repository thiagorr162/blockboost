...
command += f"bin/eval/embedding {args.path}"
...
for md, line in enumerate(lines):
    ...
    if md < len(lines) / 2:
        ...
        ponderated_md = (md - len(lines)/2) / (len(lines)/2 -1)
        out_path = (out_folder_ponderated + '/' + datafold + f"-max_dist_{md-len(lines)//2}.json")
        hparams["max_ponderated_hamming_distance"] = ponderated_md
